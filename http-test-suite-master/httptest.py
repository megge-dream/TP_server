...
class HttpServer(unittest.TestCase):
  ...
  def test_document_root(self):
    """document root escaping"""
    self.conn.request("GET", "/httptest/../httptest/dir2/page.html")
    r = self.conn.getresponse()
    data = r.read()
    self.assertEqual(int(r.status), 200)

  def test_document_root_bad(self):
    """document root escaping bad"""
    self.conn.request("GET", "/httptest/../../../cling_android/cling_android.iml")
    r = self.conn.getresponse()
    data = r.read()
    self.assertEqual(int(r.status), 403)
